...
- Name: antiquotar.spec.ts >> Tier 1: Feature Coverage >> 2. Import Netscape format cookie
- Location: tests\antiquotar.spec.ts:80:3
...
```ts
  ...
  2   | 
  3   | // Helper to add a session
  4   | async function addSession(page, { inbox, domain, label, format, used, limit }) {
  5   |   await page.fill('#cookie-inbox-input', inbox);
  6   |   await page.fill('#cookie-domain-input', domain);
  7   |   if (label !== undefined) {
  8   |     await page.fill('#cookie-name-input', label);
  9   |   }
